...
def EditMenu(state, id, username, firstname, type, text):
    ...
    if(state == 42):
        if(text == "Add"):
            ...
            dishes = dbconnection.executeQuery("SELECT dish_name FROM dishes WHERE restaurant_id= %s", [restID[0]], True)
            dishesNames = objectList(dishes)
            messageHandling.sendKeyboard(id,"what dish do you want to delete?", {"keyboard": [dishesNames], "one_time_keyboard":True}) #crear un customkye
            dbconnection.saveUserState(id, 46)
        elif(text == "Cancel"):
            messageHandling.sendMessage(id, "Command has been canceled")
            dbconnection.saveUserState(id, 0)
    if(state == 43):
        global dishName
        dishName = text
    ...
    if(state == 45):
        ...
        messageHandling.sendMessage(id, "The dish" + dishName + "has been added.")
        dbconnection.saveUserState(id, 41)
    ...
    if(state == 46):
        ...
        messageHandling.sendMessage(id, "The dish" + dishName + "has been erased.")
        dbconnection.saveUserState(id, 41)
        identifyCommand("/editmenu", 41, id, username, firstname, type, restaurantName)
...
def DishDescription(state, id, username, firstname, type, text):
    ...
    if (state == 62):
           ...
           dishName = text
           messageHandling.sendMessage(id, "Type the description to "+ dishName)
           dbconnection.saveUserState(id, 63)
    if (state == 63):
           dishDescription = text
